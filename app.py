...
water = st.sidebar.number_input("Design Plant Capacity (LPD)", value=5000, step=500)
...
# Module Sizing
safety_factor = tank_safety_margin  # Combining buffer allocation
...
# NEW!! SECTION 4: INPUT-DRIVEN DYNAMIC P&ID SCHEMATIC Diagram
st.markdown('<h3 class="section-header">📐 IV. Live Schematic & Process Flow Blueprint (P&ID Based)</h3>', unsafe_allow_html=True)
st.markdown(" This architectural model updates line sizes, fluid parameters, loop architectures, and volumes live based on your sidebar configurations.")

# Render schematic dynamically using template strings matching variable memory hooks
st.graphviz_chart(f"""
digraph G {{
    rankdir=LR;
    node [shape=box, style="filled,rounded", fontname="Helvetica-Bold", color="#0f52ba", fillcolor="#f1f3f5", fontsize=11, penwidth=2];
    edge [fontname="Helvetica", fontsize=10, color="#457b9d", penwidth=2];
    
    Inlet [label="Cold Water Feed\\n({tin}°C)", fillcolor="#e3f2fd", shape=cds];
    Tank [label="Thermal Storage Tank\\n({storage_tank_capacity:,.0f} Liters\\nSS316 Insulation)", shape=cylinder, fillcolor="#fff3cd"];
    Pump [label="Circulation Loop\\n{pump_hp} VFD Pump", shape=component, fillcolor="#e8f5e9"];
    Array [label="SQS Collector Array\\n({modules} Modules in {total_banks} Banks)\\nField: {total_collector_area:.1f} m²", fillcolor="#ffebee"];
    Process [label="Factory Process Delivery\\n({tout}°C @ {flow_lpm:.1f} LPM)", fillcolor="#e8f5e9", shape=cds];

    Inlet -> Tank [label="Makeup Loop"];
    Tank -> Pump [label="VFD Suction Line"];
    Pump -> Array [label="{flow_kghr:.1f} kg/hr\\nTarget Velocity"];
    Array -> Tank [label="Thermal Return Line\\n(Gain ΔT via Solar)"];
    Tank -> Process [label="Process Load Delivery"];
}}
""")

# SECTION 5: LIVE CONTROLLER MATRIX
...
with col_logic:
    ...
    | :--- | :--- |
    | **Current Controller Target State** | **{zone}** (System Design: {flow_kghr:.1f} kg/hr) |
