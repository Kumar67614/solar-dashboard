...
# HIGH-FIDELITY PARALLEL PROCESS PLANT P&ID DIAGRAM
st.markdown('<h3 class="section-header">📐 III. High-Fidelity Industrial Process & Instrumentation Diagram (P&ID)</h3>', unsafe_allow_html=True)
st.markdown("This blueprint uses authentic ISA instrumentation symbology, inline check valves, safety mechanics, and parallel collection boundaries matching your engineering reference model.")
...
digraph G {{
    ...
    nodesep=0.4;
    ranksep=0.6;
    node [fontname="Helvetica", fontsize=9, penwidth=1.5];
    edge [fontname="Helvetica-Bold", fontsize=8, penwidth=1.5];

    # Inline Piping Components & Valve Graphics
    node [shape=triangle, fixedsize=true, width=0.3, height=0.2, style=filled, fillcolor="#b0bec5", color="#37474f"];
    valv_in [label=""]; valv_out [label=""]; valv_ret [label=""]; valv_by [label=""];
    
    node [shape=diamond, fixedsize=true, width=0.3, height=0.3, fillcolor="#eceff1", label="Y"];
    strainer [label=""];

    # ISA Standard Instrument Circles (Transmitters & Controllers)
    node [shape=circle, fixedsize=true, width=0.6, style=filled, fillcolor="#e0f7fa", color="#006064", fontname="Helvetica-Bold"];
    LT_101 [label="LT\\n101"];
    TT_101 [label="TT\\n101"];
    TT_102 [label="TT\\n102"];
    PT_101 [label="PT\\n101"];
    ...
    node [shape=box, style="filled,rounded", width=1.6, height=1.1, color="#0f52ba", fillcolor="#f8f9fa", fontsize=10];
    Inlet [label="📥 COLD WATER INLET\\n({tin}°C Supply Feed)", shape=cds, fillcolor="#e3f2fd"];
    
    # Dual Parallel Bank Solar Collector Array Architecture
    subgraph cluster_solar {{
        label="☀️ SOLAR RECOVERY ARRAY FIELD ({total_banks} Banks Parallel / Tilt: {latitude}°)";
        color="#c62828"; style="dashed,rounded"; bgcolor="#fffde7";
        Array_A [label="PANEL BANK A\\n({modules_per_bank} Modules Series)", fillcolor="#ffebee", color="#e53935"];
        Array_B [label="PANEL BANK B\\n({max(1, modules-modules_per_bank)} Modules Series)", fillcolor="#ffebee", color="#e53935"];
    }}

    HEX [label="🔄 HEAT EXCHANGER\\nHEX-101\\n(Shell & Tube Loop)", shape=box3d, fillcolor="#e8f5e9", color="#2e7d32"];
    Tank [label="🛢️ HOT WATER STORAGE TANK\\nTK-101\\n({storage_tank_capacity:,.0f} L Capacity)", shape=cylinder, fillcolor="#fff3cd", color="#f9a825", width=1.4, height=2.2];
    ...
    node [shape=component, width=0.7, height=0.5, fillcolor="#e8f5e9", color="#2e7d32"];
    Pump_P1 [label="P-1\\n({pump_hp})"];
    Pump_P2 [label="P-2\\n(Booster)"];

    # --- HYDRAULIC UTILITY FLOOPS ---

    # Cold Input / Sanitary Water Feed Primary Stage
    Inlet -> strainer [color="#0f52ba"];
    strainer -> valv_in [color="#0f52ba"];
    valv_in -> Tank [color="#0f52ba", label=" Makeup"];
    
    # Instrumentation tracking tank fluid columns
    Tank -> LT_101 [style=dotted, color="#546e7a", arrowhead=none];

    # Primary Closed Loop Heat Collection Path (Red Utility Piping)
    Tank -> Pump_P1 [color="#0f52ba", label=" Suction"];
    Pump_P1 -> HEX [color="#0f52ba", label=" Cold Tube-Side"];
    
    # Solar Field Splitting across parallel banks
    HEX -> Array_A [color="#457b9d", label=" Manifold Feed\\n{flow_kghr/2:.1f} kg/hr"];
    HEX -> Array_B [color="#457b9d"];
    
    # High-temperature common return collection pipeline
    Array_A -> TT_101 [color="#c62828", label=" Hot Return"];
    Array_B -> TT_101 [color="#c62828"];
    
    TT_101 -> PT_101 [color="#c62828"];
    PT_101 -> valv_out [color="#c62828"];
    valv_out -> Tank [color="#c62828", label=" Solar Thermal Harvest"];

    # Secondary Stage Hot Water Delivery & Recirculation
    Tank -> Pump_P2 [color="#c62828", label=" Hot Extraction"];
    Pump_P2 -> TT_102 [color="#c62828"];
    TT_102 -> valv_ret [color="#c62828"];
    
    valv_ret -> Boiler [color="#c62828", label=" Forward Feed"];
    valv_ret -> valv_by [color="#f57f17", label=" Recirc Loop\\n(If Temp < {tout}°C)"];
    valv_by -> Tank [color="#f9a825"];
    
    Boiler -> FT_101 [color="#c62828"];
    FT_101 -> Process [color="#c62828", label=" Process Delivery\\n({tout}°C)"];

    # --- INSTRUMENTATION AUTOMATION INTERCONNECTS ---
    edge [style=dashed, color="#757575", penwidth=1, arrowhead=dot];
    TT_101 -> TIC_101 [label=" PV1"];
    TT_102 -> TIC_101 [label=" PV2"];
    TIC_101 -> Pump_P1 [label=" Loop VFD Speed"];
    TIC_101 -> valv_ret [label=" Divert Control"];
}}
""")
