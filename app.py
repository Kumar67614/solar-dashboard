...
with c1:
    st.markdown(f'<div class="metric-container"><div class="metric-title">🔥 THERMAL ENERGY DEMAND</div><div class="metric-value">{energy:.1f} kWh/day</div></div>', unsafe_allowed_html=False)
    st.metric(label="Thermal Energy", value=f"{energy:.1f} kWh/day", label_visibility="collapsed")
with c2:
    st.markdown(f'<div class="metric-container" style="border-left-color: #28a745;"><div class="metric-title">📦 REQUIRED MODULES</div><div class="metric-value">{modules} Units</div></div>', unsafe_allowed_html=False)
    st.metric(label="Modules", value=modules, label_visibility="collapsed")
with c3:
    st.markdown(f'<div class="metric-container" style="border-left-color: #ffc107;"><div class="metric-title">📐 TOTAL COLLECTOR AREA</div><div class="metric-value">{area:.1f} m²</div></div>', unsafe_allowed_html=False)
    st.metric(label="Area", value=f"{area:.1f} m²", label_visibility="collapsed")
with c4:
    st.markdown(f'<div class="metric-container" style="border-left-color: #17a2b8;"><div class="metric-title">💧 DESIGN FLOW RATE</div><div class="metric-value">{flow_lpm:.1f} LPM</div></div>', unsafe_allowed_html=False)
    st.metric(label="Flow", value=f"{flow_lpm:.1f} LPM", label_visibility="collapsed")
...
with c5:
    st.markdown(f'<div class="metric-container" style="border-left-color: #20c997;"><div class="metric-title">💰 ESTIMATED ANNUAL SAVINGS</div><div class="metric-value">₹ {annual_savings:,.0f}</div></div>', unsafe_allowed_html=False)
    st.metric(label="Annual Savings", value=f"₹ {annual_savings:,.0f}", label_visibility="collapsed")
with c6:
    st.markdown(f'<div class="metric-container" style="border-left-color: #6f42c1;"><div class="metric-title">📉 ANNUAL CO₂ REDUCTION</div><div class="metric-value">{co2_reduction:,.0f} kg/year</div></div>', unsafe_allowed_html=False)
    st.metric(label="CO₂ Reduction", value=f"{co2_reduction:,.0f} kg/year", label_visibility="collapsed")
