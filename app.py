...
# Fixed Internal Workspace/Database Map 
REF_FILES = {
    ...
    "200 LPH Profile - Target 100°C": "21-11-2025_SQS.csv", # Double check if this matches your workspace folder
...
# Enhanced parsing engine to automatically find corrupted string fragments from messy CSV metadata
def find_col(columns, keyword):
    for col in columns:
        normalized_col = col.upper().replace(" ", "").replace("_", "")
        normalized_kw = keyword.upper().replace(" ", "").replace("_", "")
...
st.sidebar.header("🔧 Database Loop Controls")
...
# Load and process selected database reference data file
if os.path.exists(filename):
    ...
    # Clean workspace column headers
    df.columns = [c.strip() for c in df.columns]
    
    # Map essential columns dynamically using the enhanced parser
    c_time = find_col(df.columns, 'TIME') or 'Time'
    c_tin = find_col(df.columns, 'TEMP IN') or find_col(df.columns, 'TEMPIN')
    c_tout = find_col(df.columns, 'TEMP OUT') or find_col(df.columns, 'TEMPOUT')
    c_tamb = find_col(df.columns, 'AMBIENT TEMP') or find_col(df.columns, 'AMBIENT')
    c_rad = find_col(df.columns, 'IT(') or find_col(df.columns, 'W/m2') or find_col(df.columns, 'RADIATION')
    c_flow = find_col(df.columns, 'FLOW RATE') or find_col(df.columns, 'FLOWRATE')
    c_ein = find_col(df.columns, 'ENERGY INPUT') or find_col(df.columns, 'ENERGYIN')
    c_eout = find_col(df.columns, 'ENERGY OUTPUT') or find_col(df.columns, 'ENERGYOUT')
    c_eff = find_col(df.columns, 'EFFICIENCY')
    c_pin = find_col(df.columns, 'PRES IN') or find_col(df.columns, 'PRESIN')
    c_pout = find_col(df.columns, 'PRES OUT') or find_col(df.columns, 'PRESOUT')
    c_pdel = find_col(df.columns, 'DELTA PRES') or find_col(df.columns, 'DELTAPRES')
    ...
    st.markdown(f"<div class='subtitle'>Visualizing context-aware metrics for <b>{selected_profile}</b> (Source File: <code>{filename}</code>)</div>", unsafe_allow_html=True)
    ...
    # Diagnostic alert detailing precisely where python is checking for files
    st.error(f"⚠️ Reference data file `{filename}` was not found.")
    st.info(f"**Diagnostic Help:** The app is looking for `{filename}` inside your current directory: `{os.getcwd()}`. Please verify that this file is placed exactly in this path alongside your dashboard script.")
