...
water = st.sidebar.number_input("Design Plant Capacity (LPD)", value=8000, step=500)
tout = st.sidebar.number_input("Process Delivery Output Temp (°C)", value=90, min_value=30, max_value=95)
...
# SECTION 4: FINANCIAL & REVENUE GAUGES (HIGHLY PICTORIAL REPLACE TEXT)
...
with col_f1:
    ...
    fig_capex = go.Figure(go.Indicator(
        ...
        gauge={
            'axis': {'range': [0, 10000000], 'tickformat': ',', 'tickfont': {'size': 10}},
            'bar': {'color': "#1d3557"},
            'bgcolor': "#f8f9fa"
        }
    ))
...
with col_logic:
    ...
    | **Solar Irradiance Gate Gate** | ☀️ {irr} |
